...
pub fn init_background(theme: &Theme) {
    ...
    let colors: std::collections::HashMap<&str, Srgba> = [
    ...
    spawn_local(async move {
        // Window and Context      
        let window = Window::new(WindowSettings {title: canvas_id, ..Default::default()}).unwrap();
        ...
        // Instances and mesh
        let grid_parameters = get_grid_parameters(&window, h, 75);
...
async fn load_mesh(path: &str) -> CpuMesh {
    ...
    let base = if href.ends_with('/') { href.clone() } else { format!("{}/", href) };
    let url = format!("{}public/{}", base, path);

    // Load the raw asset bytes
    let mut loaded = three_d_asset::io::load_async(&[url.as_str()]).await.unwrap();
...
fn init_instances(grid_parameters: &GridParameters) -> Instances {
    ...
    instances.transformations =(0..grid_parameters.rows).flat_map(|row| {
...
fn setup_instanced_mesh(context: &Context, cpu_mesh: CpuMesh, instances: &Instances, color: Srgba) -> Gm<InstancedMesh, ColorMaterial> {
    Gm::new(
...
fn mouse_update_instance(mouse: &MouseState, instances: &mut Instances, grid_parameters: &GridParameters) {
...
fn render_frame(camera: &Camera, mesh: &Gm<InstancedMesh, ColorMaterial>, light: &AmbientLight, color: &Srgba, frame_input: FrameInput) -> FrameOutput {
    frame_input.screen()
